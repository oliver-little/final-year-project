...
\todo{high level figures showing example select, filter, group by with reference to section}

\subsection{FieldExpressions}\label{subsec:fieldexpressions}
\textit{FieldExpressions} are a key part of the DSL, allowing the user to define arbitrary row-level calculations to be used as part of more complex operations. \textit{FieldExpressions} are defined as an interface, and there are three subclasses:
...
\begin{figure}[ht]
	Values reference the type and value (left to right): string "a", integer 1, double 1.5, boolean True, date 31/12/2021.
	\begin{python}
V("a") , V(1), V(1.5), V(True), V(datetime.date(2021, 12, 31))
	\end{python}

	Fields reference the field with name (left to right): fieldName, duration, id, creationDate.
	\begin{python}
...
(F("field2") * 2) / F("field3")
	\end{python}
	\caption{\textit{FieldExpressions} implementations and examples}
...
\todo{reword to mostly use a figure with example using asname, and example of F automatic resolution}
...
Examples of \textit{FieldComparisons} are shown in Figure \ref{fig:field-comparisons-examples}. \todo{reword to explain with reference to this figure}
...
The system supports Minimum, Maximum, Sum, Average, Count, and String Concatenation operations. These aggregates are polymorphic where possible. For example, minimum and maximum handle numeric types by ordering numerically and string types lexicographically. \todo{reword into a figure example}
...
Figure \ref{fig:group-by-query} shows a more complex query containing a Group By in the DSL, and the data model. \todo{change this diagram a bit or explain better, table 1 is the dependent table of the group by query}
\begin{figure}[h]
	\centering
