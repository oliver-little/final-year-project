...
object TableSizeEstimation {
    def estimateTableSize(session : CqlSession, keyspace : String, table : String) : TableSizeEstimation = {
        ...
        res.iterator.asScala.map(row => SizeEstimatesRow(CassandraTokenRange.fromLong(session.getMetadata.getTokenMap.get, row.getLong("range_start"), row.getLong("range_end"), row.getLong("mean_partition_size"), row.getLong("partitions_count"))))
    }
}
