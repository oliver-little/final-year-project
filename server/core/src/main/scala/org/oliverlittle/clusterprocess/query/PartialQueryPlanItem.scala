...
object PartialQueryPlanItem:
    def fromProtobuf(item : worker_query.QueryPlanItem, connector : CassandraConnector) : PartialQueryPlanItem = item.item match {
        ...
        case worker_query.QueryPlanItem.Item.PreparePartition(worker_query.PreparePartition(Some(dataSource), numPartitions, unknownFields)) => PartialPreparePartition(DataSource.fromProtobuf(dataSource), numPartitions)
...
case class PartialPreparePartition(dataSource : DataSource, numPartitions : Int) extends PartialQueryPlanItem:
